...
def heat(old_path, node_w, m, D, v, c, start_time, end_time, service_time):
    ...
    """
    dc = total_cost(old_path, node_w, m, D, v, c, start_time, end_time, service_time)
    T0 = 20 * dc  # 将初始温度设置为最大偏差的20倍

    return T0, old_path

#贪婪算法求初始解
def greedy_init(D, num_city):
    result = []
    ...
    while len(rest) != 0:
        ...
        tmp_choose = -1
        for x in rest:
            if D[current][x] < tmp_min:
                tmp_min = D[current][x]
                tmp_choose = x
        current = tmp_choose
        result.append(tmp_choose)
        rest.remove(tmp_choose)
    print("greedy_result:",result)
    return result
...
def init(n, m):
    ...
    #生成初始解
    init_path=greedy_init(D,n)
    init_path.append(0)
    #将仓库以相同间隔插入初始解序列中
    index = 0
    for i in range(m-1):
        index = index + (m+1)
        init_path.insert(index,0)
